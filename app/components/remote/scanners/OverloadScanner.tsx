...
export const OverloadScanner = ({ projectLocalPath }: OverloadScannerProps) => {
    ...
    return (
        <Popover placement="bottom-end" isOpen={isOpen} onOpenChange={setIsOpen} shouldFlip>
            ...
            <PopoverContent className="p-0 w-[500px] max-h-[600px] flex flex-col shadow-xl border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div className="flex flex-col gap-1">
                        {files.map((file) => (
                            <button
                                ...
                                className="group flex items-start gap-3 px-3 py-2 hover:bg-orange-50/50 rounded-md transition-colors text-xs text-left w-full border border-transparent hover:border-orange-100"
                            >
                                <div className="mt-1 text-gray-400 group-hover:text-orange-500 flex-shrink-0">
                                    <FileCode size={14} />
                                </div>
                                <div className="flex-grow min-w-0 flex flex-col gap-0.5">
                                    <div className="flex justify-between items-center gap-2">
                                        <div className="font-mono text-gray-700 font-medium truncate group-hover:text-gray-900 text-[11px]" title={file.fileName}>
                                            {file.fileName}
                                        </div>
                                        <div className="text-orange-600 font-mono font-bold bg-orange-50 px-1.5 py-0.5 rounded border border-orange-100 group-hover:bg-orange-100 text-[10px] shrink-0">
                                            {file.lineCount}
                                        </div>
                                    </div>
                                    <div className="text-[10px] text-gray-400 truncate font-mono opacity-60">
                                        {file.absolutePath.replace(scannedPath, '')}
                                    </div>
                                </div>
                            </button>
                        ))}
