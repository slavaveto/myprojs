...
export const RichEditableCell = ({
    ...
    const [isEditing, setIsEditing] = useState(true); // Always render Editor for now to fix alignment issues
    ...
    /* Disabled View Mode for debugging/alignment fix */
    /*
    ...
    if (!isEditing) {
        return (
            <div 
                ...
                className={clsx(
                    "cursor-text min-h-[24px] outline-none break-words whitespace-pre-wrap leading-normal flex items-center [&_p]:m-0", 
                    !content && "text-default-300",
                    className
                )}
    ...
    return (
        <TiptapEditor 
            ...
            onSave={(newContent: string) => {
                ...
                onSave(newContent);
                // setIsEditing(false); // Don't switch off editing
            }}
            onCancel={() => {
                // setIsEditing(false);
...
const TiptapEditor = ({ 
    ...
    useEffect(() => {
        ...
        // Custom keyboard handler for Enter to Save
        ...
        // editor.view.dom.addEventListener('keydown', handleKeyDown); // Handled by editorProps
    }, [editor, onSave]);
    ...
    const handleBlur = useCallback(() => {
        if (editor) {
            ...
            // We compare against initialContent prop passed to EditorWrapper
